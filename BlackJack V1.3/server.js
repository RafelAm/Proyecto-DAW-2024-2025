...
import { Crupier, Jugador, Partida } from "./party.js"; // Importa la logica del juego.
...
/**
 * Inicializa las partidas base al arrancar el servidor.
...
/**
 * Evento principal de conexión de sockets.
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", (roomId) => {
    ...
    /**
     * Evento para solicitar el estado del juego.
     * @event socket.on('gameStateRequest')
     * @param {Object} data - Datos con roomId.
     * @author Rafel Amengual Tomás
     * @date 2024-05-28
     */
  socket.on("gameStateRequest", ({ roomId }) => {
    ...
    if (game.reiniciando) {
      return socket.emit(
        "error",
        "La partida está reiniciándose, espera unos segundos."
      );
    }
  ...
  socket.on("requestCard", ({ roomId }) => {
    ...
    const game = games[roomId];
    if (!game || game.reiniciando)
      return socket.emit("error", "La partida está en proceso de reinicio.");

    const username = socket.data.username;
    const playerIndex = game.jugadores.findIndex(
      (player) => player.nombre === username
    );
    if (playerIndex === -1)
      return socket.emit("error", "No se encontró el jugador en la partida.");

    // CORRIGE AQUÍ:
    if (game.turnoActual !== playerIndex)
  ...
  socket.on("plantarse", ({ roomId }) => {
    ...
    const quedanJugadores = game.jugadores.some(
      (j) => j.tipo === "Player" && !j.plant
    );
    if (quedanJugadores) {
      game.turnoActual = game.jugadores.findIndex(
        (j) => j.tipo === "Player" && !j.plant
      );
    } else {
    ...
    /**
     * Evento para añadir jugador a la partida.
     * @event socket.on('addPlayer')
     * @param {Object} data - roomId, username, socketId.
     * @author Rafel Amengual Tomás
     * @date 2024-05-28
     */
  socket.on("addPlayer", async ({ roomId, username, socketId }) => {
    ...
    // Si el jugador ya está en la partida, no hagas nada
    if (game.jugadores.some(j => j.nombre === username)) {
    ...
    if (game.empezada) {
      return socket.emit(
        "error",
        "La partida ya ha comenzado y no se pueden unir más jugadores."
      );
    }
    ...
    if (game.jugadores.length < MAX_JUGADORES && !game.empezada) {
      try {
        ...
        const [rows] = await db.query(
          "SELECT id, dinero FROM usuarios WHERE nombre = ?",
          [username]
        );
        if (!rows.length) {
          return socket.emit(
        ...
        const crupierIndex = game.jugadores.findIndex(
          (j) => j.tipo === "Crupier"
        );
        ...
        const jugadoresHumanos = game.jugadores.filter(
          (jugador) => jugador.tipo === "Player"
        );
        // SOLO inicia la cuenta atrás si no está activa
     ...
     * @event socket.on('realizarApuesta')
     * @param {Object} data - roomId, monto, fichas.
     * @author Rafel Amengual Tomás
     * @date 2024-05-28
     */
  socket.on("realizarApuesta", async ({ roomId, monto, fichas }) => {
    ...
    if (game.jugadores[playerIndex].balanceInicial === undefined) {
      game.jugadores[playerIndex].balanceInicial =
        game.jugadores[playerIndex].balance;
    }
    ...
    try {
      ...
      if (monto > dineroActual)
        return socket.emit(
          "error",
          "No tienes suficiente dinero para realizar esta apuesta."
        );

      game.jugadores[playerIndex].apuesta = monto;
      game.jugadores[playerIndex].fichasApostadas = Array.isArray(fichas)
        ? fichas
        : [];
      game.jugadores[playerIndex].balance -= monto;

      dineroActual -= monto;
      await db.execute("UPDATE usuarios SET dinero = ? WHERE nombre = ?", [
        dineroActual,
        username,
      ]);
      ...
      if (game.countDown === true) {
        ...
        // Ahora sí, emite el estado actualizado a todos
        emitirGameStateATodos(roomId, game, io);
      }
  ...
  socket.on("disconnect", async () => {
     ...
     * @event socket.on('chat message')
     * @param {Object} data - roomId, message, username.
     * @author Rafel Amengual Tomás
     * @date 2024-05-28
     */
  ...
  /*------------------------FUNCIONES------------------------*/
    ...
    /**
     * Muestra el formulario de apuesta a los jugadores que no han apostado.
     * @function mostrarFormularioApuesta
     * @param {string|number} roomId - ID de la sala.
     * @param {Partida} game - Instancia de la partida.
  ...
  function mostrarFormularioApuesta(roomId, game, io) {
    ...
    /**
     * Emite el estado filtrado del juego a todos los sockets de una sala.
     * @async
     * @function emitirGameStateATodos
     * @param {string|number} roomId - ID de la sala.
     * @param {Partida} game - Instancia de la partida.
     * @param {object} io - Instancia de Socket.IO.
     * @author Rafel Amengual Tomás
...
/**
 * Emite el estado filtrado del juego a un socket concreto.
 * @async
 * @function emitirGameState
 * @param {object} emisor - Socket emisor.
 * @param {Partida} game - Instancia de la partida.
 ...
 */
  async function emitirGameState(emisor, game, socketId, username) {
    ...
    if (nombresJugadores.length > 0) {
      const [rows] = await db.query(
        `SELECT nombre, imagenPerfil FROM usuarios WHERE nombre IN (${nombresJugadores
          .map(() => "?")
          .join(",")})`,
        nombresJugadores
      );
      rows.forEach((row) => {
        if (row.imagenPerfil) {
          imagenesPerfil[row.nombre] = `data:image/png;base64,${Buffer.from(
            row.imagenPerfil
          ).toString("base64")}`;
        }
      });
...
/**
 * Inicia la cuenta atrás para unirse y apostar en la partida.
 * @async
 ...
 * @param {object} io - Instancia de Socket.IO.
 * @returns {Promise<number|null>} ID de la partida o null si se cancela.
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
      ...
      try {
        ...
        setTimeout(async () => {
          ...
          setTimeout(async () => {
            ...
            game.jugadores = game.jugadores.filter(
              (jugador) => jugador.tipo === "Crupier" || jugador.apuesta > 0
            );
            ...
            if (game.jugadores.length >= 1) {
              ...
              for (const jugador of game.jugadores.slice(
                0,
              ...
              let idCrupier;
              const [nuevoCrupier] = await db.execute(
                `INSERT INTO crupier (derrotas, victorias) VALUES (0, 0)`
              );
              ...
              io.to(roomId).emit(
                "error",
                "No se ha alcanzado el número mínimo de jugadores con apuestas. Partida cancelada."
              );
              resolve(null);
            }
        ...
        console.error(
          "❌ Error al registrar la partida en la base de datos:",
          error
        );
        io.to(roomId).emit(
          "error",
...
/**
 * Inicia el temporizador del turno para el jugador actual.
...
/**
 * Actualiza el turno del juego, pasando al siguiente jugador o al crupier si todos han terminado.
 * @function actualizarTurnoJuego
 * @param {Partida} game - Instancia de la partida.
 * @param {string|number} roomId - ID de la sala.
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
  function actualizarTurnoJuego(game, roomId) {
    ...
    if (quedanJugadores) {
      ...
      do {
        ...
        (game.jugadores[siguiente].tipo !== "Player" ||
          game.jugadores[siguiente].plant) &&
  ...
  /**
 * Verifica si todos los jugadores han terminado su turno y si el crupier debe jugar.
 * @function verificarFinalRound
 * @param {string|number} roomId - ID de la sala.
 * @param {Partida} game - Instancia de la partida.
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
  ...
  /**
 * Maneja el final de la ronda: reparte premios, actualiza estadísticas y reinicia la partida.
 * @async
 * @function manejarFinalRound
 * @param {string|number} roomId - ID de la sala.
 * @param {Partida} game - Instancia de la partida.
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
    ...
    try {
      ...
      for (const jugador of game.jugadores.filter((j) => j.tipo === "Player")) {
        ...
        const empate =
          !gano &&
          jugador.puntaje ===
            game.jugadores[game.jugadores.length - 1].puntaje &&
          jugador.puntaje <= 21;
        ...
        if (gano) {
          ...
          dineroGanado =
            jugador.balance - balanceInicial > 0
        ...
        const [rows] = await db.query(
          "SELECT * FROM estadisticasusuario WHERE idUsuario = ?",
          [jugador.id]
        );
        if (rows.length === 0) {
        ...
        await db.execute(`UPDATE usuarios SET dinero = ? WHERE id = ?`, [
          jugador.balance,
          jugador.id,
        ]);
      ...
      let crupierVictoria = game.ganadores.some((g) => g.nombre === "Crupier")
        ? 1
        : 0;
...
app.use(async (req, res, next) => {
  ...
  if (req.session.username) {
    try {
      const [rows] = await db.query(
        "SELECT rol, imagenPerfil FROM usuarios WHERE nombre = ?",
...
app.get("/dashboard", checkAuth, checkAdmin, async (req, res) => {
  try {
    const partidas = await db.query(
      "SELECT * FROM partida ORDER BY fecha_partida DESC LIMIT 10"
    );
    const jugadores = await db.query(
      "SELECT id, nombre, correo, dinero, rol FROM usuarios ORDER BY nombre ASC"
    );
...
/**
 * API que devuelve la información de una partida específica.
...
app.get("/api/game/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener la baraja de la partida." });
  }
});

/**
 * Verifica si todas las partidas están llenas y crea una nueva si es necesario.
...
app.post("/register", upload.single("imagenPerfil"), async (req, res) => {
  ...
  try {
    const [rowsNombre] = await db.query(
    ...
    const [rowsCorreo] = await db.query(
    ...
    await db.execute(
      ...
      [nombre, correo, contraseñaHash, 30, "Jugador", imagenPerfil]
    );
 ...
 * @route GET /verificar-nombre
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
app.get("/verificar-nombre", async (req, res) => {
  const { nombre } = req.query;
  const [rows] = await db.query("SELECT id FROM usuarios WHERE nombre = ?", [
    nombre,
  ]);
 ...
 * @route GET /verificar-correo
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
app.get("/verificar-correo", async (req, res) => {
  const { correo } = req.query;
  const [rows] = await db.query("SELECT id FROM usuarios WHERE correo = ?", [
    correo,
  ]);
  res.json({ existe: rows.length > 0 });
});
...
/**
 * Verifica si el usuario y el correo ya existen.
 * @route GET /verificar-usuario
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
...
app.post("/login", async (req, res) => {
  ...
  if (!regexCorreo.test(correo) && correo.length < 3) {
    return res.json({
      error: true,
      mensaje: "Introduce un correo válido o un nombre de usuario válido.",
    });
  }
  ...
  try {
    ...
    if (!usuario) {
      return res.json({
        error: true,
        mensaje:
          'Usuario no encontrado. ¿Quieres <a href="/register">registrarte</a>?',
      });
    }
...
app.post("/update-user", async (req, res) => {
  ...
  try {
    await db.query("UPDATE usuarios SET dinero = ? WHERE id = ?", [
      dinero,
      userId,
    ]); // Ahora usa el ID de usuario seleccionado
...
app.post("/create-user", async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "INSERT INTO usuarios (nombre, correo, contraseña_hash, dinero, rol) VALUES (?, ?, ?, ?, ?)",
...
/**
 * Obtener información de usuario o partida.
 * @route GET /get-info/:type/:id
 * @author Rafel Amengual Tomás
 * @date 2024-05-28
 */
...
async function checkAdmin(req, res, next) {
  ...
  const [rows] = await db.query("SELECT rol FROM usuarios WHERE nombre = ?", [
    req.session.username,
  ]);
...
app.post("/shop/pay", checkAuth, upload.none(), async (req, res) => {
  ...
  try {
    await db.query("UPDATE usuarios SET dinero = dinero + ? WHERE nombre = ?", [
      parseInt(coins, 10),
      req.session.username,
    ]);
    return res.json({
...
/**
 * API que devuelve la información completa de una partida.
...
app.get("/api/partida/:id", async (req, res) => {
  ...
  try {
    ...
    const [partidaRows] = await db.query("SELECT * FROM partida WHERE id = ?", [
      id,
    ]);
    ...
    res
      .status(500)
      .json({ error: "Error al obtener los datos de la partida." });
  }
});
